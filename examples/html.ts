...
const htmlParser = () => {
  ...
  const attributes = P.flat(P.many(P.seq(
    ws,
    P.choice(
      P.flat(P.seq(
        identifier,
  ...
  const tagClose = empty(P.seq(angleBracketOpen, slash, identifier, ws, angleBracketClose))
  ...
  const tag = P.flat(P.seq(
    tagOpen,
    P.unit(body.getParser()),
    tagClose
  ))

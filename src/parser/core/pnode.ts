...
export type PNodeMapper<A, B> = PNodeMatchMapper<A, B> | PNodeMatchMapperDefault<A, B>
...
export class PNodes<A = {}> implements IPNodeMatch<A>, IPNodeMatchMap<A>, IPNode<A> {
  ...
  forEach(callbackfn: (value: PNode<A>, index: number, array: PNode<A>[]) => void) {
...
function recursiveFmt(node: PNode<{}>, indent: string, isLast: boolean, isRoot: boolean): string {
    let res = ""

...
class Debugger {
  ...
  private recursivelyAnalyze(
    ...
    if (!isSuccess) {
      res += `${isSuccess ? "S" : "F"} ${graph.value.type} - p: ${
        graph.value.state.position
      }`;
      const lastIndex = graph.children.length - 1;
      graph.children.forEach((child, i) => {
        res += "\n";
        res += this.recursivelyAnalyze(
          child,
          nextDepth,
          i === lastIndex,
          false
...
export const debug = (process: DebugProcess) => <A>(
  parser: Parser<A>
): Parser<A> => {
